...
export class SecretsManagerContentComponent implements OnInit, OnDestroy {
  ...
  private paidCallouts = [
    "Sign up today and receive a complimentary 12-month subscription to SaviVault Password Manager",
  ...
  private freeCallouts = [
    "Your SaviVault account will also grant complimentary access to SaviVault Password Manager",
    ...
    this.activatedRoute.queryParams.pipe(takeUntil(this.destroy$)).subscribe((queryParameters) => {
      switch (queryParameters.org) {
        case "enterprise":
          ...
          break;
        case "free":
          this.header = "SaviVault Secrets Manager";
